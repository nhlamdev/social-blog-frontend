...
interface PaginationDirectComponentProps {
  current: number;
  total: number;
  urlDirect: (page: number) => string;
}

export const PaginationDirectComponent = (
  props: PaginationDirectComponentProps
) => {
  const { current, total, urlDirect } = props;

  const paginationItem = (page: string | number) => {
    return (
      <Link
        href={urlDirect(Number(page))}
